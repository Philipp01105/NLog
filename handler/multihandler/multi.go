...
type MultiHandler struct {
	...
	fastHandlers []handler.FastHandler // cached FastHandler interfaces (nil when handler doesn't implement it)
...
func NewMultiHandler(handlers ...handler.Handler) *MultiHandler {
	m := &MultiHandler{
		handlers:     handlers,
		fastHandlers: make([]handler.FastHandler, len(handlers)),
		allFast:      true,
		recycleEntry: true,
	}
	for i, h := range handlers {
		if fh, ok := h.(handler.FastHandler); ok {
			m.fastHandlers[i] = fh
		} else {
			m.allFast = false
		}
...
func (h *MultiHandler) HandleLog(t time.Time, level core.Level, msg string, loggerFields, callFields []core.Field, caller core.CallerInfo) error {
	...
	for i, hdlr := range h.handlers {
		...
		} else if err := hdlr.Handle(entry); err != nil {
			lastErr = err
		}
	}
	if h.recycleEntry {
...
func (h *MultiHandler) Handle(entry *core.Entry) error {
	...
	for _, hdlr := range h.handlers {
		if err := hdlr.Handle(entry); err != nil {
			lastErr = err
		}
	}
	return lastErr
